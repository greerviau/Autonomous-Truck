...
def train(X_train, Y_train, X_val, Y_val, save_path, epochs=20, batch_size=8, opt=None, lss=None, save=None, retrain=False):
    ...
    with tf.Session() as sess:
        ...
        for epoch in range(epochs):
            ...
            #Randomly shift brightness of 10% of training samples
            samples_shifted = 0
            avg_shift = []
            for i in range(len(X_train)):
                if rnd.random() < 0.1:
                    ...
                    hsv = cv2.cvtColor(X_train[0].astype(np.uint8), cv2.COLOR_BGR2HSV).astype(np.int16)
                    ...
                    v[v < 0] = 0
                    hsv = cv2.merge((h, s, v))
                    X_train[0] = cv2.cvtColor(hsv.astype(np.uint8), cv2.COLOR_HSV2BGR).astype(np.float16)
...
def test(network, saver, save_path):
    #Testing
    X = np.load('data/roof_cam/processed/Peterbilt/X.npy')
    Y = pd.read_csv('data/roof_cam/processed/Peterbilt/Y.csv')['Left X'].to_numpy()
...
def main():
    ...
    print('Loading Data...')
    '''
    #X.append(np.load('data/roof_cam/processed/X.npy'))
    #Y.append(pd.read_csv('data/roof_cam/processed/Y.csv')['Left X'].to_numpy())

    X = np.load('data/roof_cam/processed/Peterbilt/X.npy')
    Y = pd.read_csv('data/roof_cam/processed/Peterbilt/Y.csv')['Left X'].to_numpy()
    '''
    for session in os.listdir(data_dir):
        X.append(np.load(os.path.join('data/roof_cam/processed/', session, 'X.npy')))
        Y.append(pd.read_csv(os.path.join('data/roof_cam/processed/', session, 'Y.csv'))['Left X'].to_numpy())
    ...
    train(X_train, Y_train, X_test, Y_test, 'conv_net_models/conv_net_v16_aug_bright_all', epochs=500, batch_size=16, opt=optimizer, lss=loss, save=saver, retrain=False)
    
    test(network, saver, 'conv_net_models/conv_net_v16_aug_bright_all')
